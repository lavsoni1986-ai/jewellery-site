        ...
        {!loading && !error && (
          <div className="grid md:grid-cols-4 gap-6">
            {filteredProducts.map((product) => {
              const price = Math.round((product.weight * (goldRate || 0)) + product.making);
              return (
                <div key={product.id} onClick={() => setSelectedProduct(product)} className="bg-black border border-[#D4AF37]/20 rounded-xl overflow-hidden shadow-lg hover:shadow-2xl transition duration-300 group cursor-pointer">
                  <div className="relative overflow-hidden">
                    <Image
                      src={product.image}
                      alt={product.name}
        ...
        {selectedProduct && (
          <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50">
            <div className="bg-[#111111] p-6 rounded-xl max-w-md w-full mx-4">
              ...
              <p className="text-[#D4AF37] text-2xl font-bold mb-4">₹{Math.round((selectedProduct.weight * goldRate) + selectedProduct.making)}</p>
              <div className="flex gap-2">
                <a
                  href={`https://wa.me/${PHONE}?text=Interested in ${selectedProduct.name}. Price: ₹${Math.round((selectedProduct.weight * goldRate) + selectedProduct.making)}\n\nGSTIN: 23DKHPS2997L1ZD`}
                  onClick={() => saveLead(selectedProduct)}
                  className="flex-1 bg-[#25D366] text-white py-2 rounded text-center"
                >
                  WhatsApp पर पूछें 💬
                </a>
                <button onClick={() => setSelectedProduct(null)} className="flex-1 border border-[#D4AF37] text-[#D4AF37] py-2 rounded">
                  Close
                </button>
              </div>
            </div>
